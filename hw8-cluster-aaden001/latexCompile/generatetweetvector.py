...
api = setup_api("secrets.json")

apcount = {}      # number of accounts each word appears in
wordcounts = {}   # words and frequency in each account
sumcounts = {}    # words and frequency over all accounts (to determine top 1000)
...
"""
Did some testing to figure out the datatype
and sample output for one acccount
Spyder ide  gives flexibility of running each variable without running the whole code after your ran the whole code at least once
"""
#print("Done Counting words")
#print(type(sumcounts))
...
# Save only the 1000 most frequent terms over all accounts (see sumcounts) in popularlist
popularlist = []
#
...
listofTuples = sorted(sumcounts.items() ,  key=lambda x: x[1],reverse=True)
#extract only 1000 rows 
listofTuples = listofTuples[:1000]
#store in the dictionary
popularlist = [i[0] for i in listofTuples]

print("Writing it to text file")
